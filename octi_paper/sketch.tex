...
We now add binary decision variables $\bend{e}{f}{0}, \dots, \bend{e}{f}{7}$ for each of the 8 possible values of $\dirdiff{u}{f}$ and the following constraint for each pair of adjacent input edges:
%
\begin{equation} 
...
\begin{figure}
  \centering
	\includegraphics[width=0.229\textwidth]{figures/node_order.pdf}
	\hfill
...
To preserve the input embedding, we have to make sure that the shortest paths never cross each other, and that the initial circular ordering of edges at nodes is preserved.
...
We assume the rendering order at $v$ to be $(e, f, g$), and paths $p(e)$ and $p(f)$ have already been routed (this means that $\gv$ is already settled).
...
To achieve this, we close each sink edge $\gse{}{}{p}$ that lies between $\gse{}{}{0}$ prior to routing $g$ by setting its cost to $\infty$ (including the sink edges used by $p(e)$ and $p(f)$) (Fig.~\ref{FIG:paths}.2a).
