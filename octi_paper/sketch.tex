...
To be able to distinguish the different node and edge types, we define the set of original grid nodes as $V^s_\omega \ni v_{x, y}$, the set of port nodes as $V^p_\omega \ni v_{x,y}^i$, the set of turn edges as $E^t_\omega \ni e_{x, y}^{i@\theta}$ and the set of sink edges as $E^s_\omega \ni e_{x,y}^i$.
