...
We assume the rendering order at $v$ to be $(e, f, g$), and paths $p(e)$ and $p(f)$ have already been routed (in particular, this means that $\gv$ is already settled and turn-closed).
The path for $e$ uses the sink edge $\gse{}{}{0}$, the path for $f$ uses sink edge $\gse{}{}{3}$.
To respect the input edge ordering at $v$ in the final drawing, we have to make sure that $p(g)$ does not enter or leave $\gv$ in the area marked red.
For this, we close each sink edge $\gse{}{}{p}$ that lies between $\gse{}{}{0}$ prior to routing $g$ by setting its cost to $\infty$ (including the sink edges used by $p(e)$ and $p(f)$) (Fig.~\ref{FIG:paths}.2a).
...
To prevent cases like this, we make sure that there are always enough sink edges left in both directions of an already routed path at $v$.
...
This means that the costs of all turn edges adjacent to $\gv$ have been set to $\infty$, which is why they are not depicted in Figures~\ref{FIG:paths}.3a and \ref{FIG:paths}.3b.
We can thus be sure that our sink edge cost are not circumvented, for example, by a path $p(g)$ using $\gse{}{}{2}$ in Figure~\ref{FIG:paths}.3b, and then using the $45^{\circ}$ turn cost connecting port $\gpv{}{}{2}$ and $\gpv{}{}{1}$.
...
Just as in the ILP formulation, our shortest path calculations so far only optimize line bends along paths and we have to additionally make sure that line bends at nodes are also penalized.
We handle this similar to the edge ordering constraints in the previous section.
If a grid node $\gv$ with has already been settled for an input node $\iv$ with adjacent edges (in rendering order) $\ie_0, ..., \ie_i, ..., \ie_{\deg(v)}$, we calculate for each $\ie_i$, $i > 0$ the line bend penalty between each already routed edge $\ie_j$, $j < i$ at $\gv$ for each of the unused adjacent sink edges prior to routing $\ie_i$.
The sum of the line bend penalty for each preceeding edge is then used as the cost for the respective sink edge (Figure~\ref{FIG:linebends}).

\subsection{Complexity}


\subsection{Further Optimization via Steepest Hill Climbing}
...
In particular, this means that the optimal drawing might not be included in this set.
...
Our approach so far does not consider this.

Despite these drawbacks, our evaluation shows that our approach can come very close to the optimal solution after just a few hill climbing iterations.
