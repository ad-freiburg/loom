...
We extend our cost function and now want to search for the path $\gPath = (\gv_0, \gv_1, ..., \gv_i, ..., \gv_n)$ which minimizes
\begin{align}
...
\begin{figure}[h]
  \centering
	...
	\caption{A $5\times3$ octilinear grid graph. Each node $\ggv{x}{y}$ has 8 ports $\gpv{x}{y}{0} ... \gpv{x}{y}{7}$ which are connected to $\ggv{x}{y}$ by direct sink edge $\gse{x}{y}{0} ... \gse{x}{y}{7}$. Each port is additionally connected to its $180^{\circ}$, $135^{\circ}$, $90^{\circ}$ and $45^{\circ}$ neighbor ports by bend edges $\gbe{x}{y}{i}{j}$.}
	\label{FIG:gridgraph}
\end{figure}
...
\begin{figure}
  \centering
	$\vcenter{\hbox{\includegraphics[width=0.474\textwidth]{figures/path.pdf}}}$
...
We assume the rendering order at $v$ to be $(e, f, g$), and paths $p(e)$ and $p(f)$ have already been routed (in particular, this means that $\gv$ is already settled and bend-closed).
...
To prevent cases like this, we make sure that there are always enough sink edges left in both directions.
...
This means that the costs of all bend edges adjacent to $\gv$ have been set to $\infty$, which is why they are not depicted in Figures~\ref{FIG:paths}.3a and \ref{FIG:paths}.3b.
