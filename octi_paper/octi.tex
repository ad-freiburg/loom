...
Given an undirected labeled input graph $\iG = (\iV, \iE, L)$, where $\iV$ are stations, $\iE$ are connections between those stations and each edge $\ie \in \iE$ is labeled with a subset $L(e) \in \mathcal{L}$ of lines traveling on this edge.
We call $\iG$ the $\emph{line graph}$.
We say $\drawing{\iG} = (\drawingpossym, \drawingcurvesym)$ is a drawing of $\iG$, where $\drawingpos{\iv} \in \mathbb{R}^2$ assigns a position to every node $\iv \in \iV$ and $\drawingcurve{\ie} = (q_0, q_1, \dots, q_n), q_i \in \mathbb{R}^2$ assigns a piece-wise linear curve to every edge $\ie \in \iE$.
...
\item \emph{Map Density}. There should be a minimum distance $\gmind$ between the stations and curve anchor points.
...
We state the problem as finding the optimal positions $\drawingpos{\iv} \in \mathbb{N}^2$ on a grid for each station node and curves $\drawingcurve{\ie} = (q_0, q_1, \dots, q_n)$, $q_i \in \mathbb{N}^2$ connecting them. 
...
To later be able to optimize soft constraint (1) between stations, we additionally want the total cost for a path in $\Gamma$ to reflect the number and acuteness of bends.
...
\begin{align}
	\gPcost{\gPath} = (n - 1) \cdot \gHopcost + \sum_{i=1}^{n - 1} c_b(\gv_{i-1}, \gv_{i+1}),
\end{align}
where $c_b(\gv_{i-1}, \gv_{i+1})$ is the angular bend cost between edges $\{\gv_{i-1}, \gv_{i}\}$ and $\{\gv_{i}, \gv_{i+1}\}$, that is, either 0, $\gPturn{135}$, $\gPturn{90}$ or $\gPturn{45}$.

\begin{figure}
  \centering
	$\vcenter{\hbox{\includegraphics[width=0.474\textwidth]{figures/grid.pdf}}}$
	\caption{Left: A shortest path between $t$ and $u$ on a grid graph with uniform edge cost. Path bends are not minimized. Right: Two shortest paths for $(t, u)$, and $(v, w)$ on our octilinear grid graph with uniform grid edge cost $1$ and additional path bend penalties $\gPturn{135} = 1$, $\gPturn{90} = 1.5$ and $\gPturn{45} = 2$. Path $(t, u)$ acts as an obstacle for $(v, w)$.}
	\label{FIG:grids}
\end{figure}
...
For paths passing through $\ggv{x}{y}$, we connect each port $\gpv{x}{y}{i}$ with its 7 - i succeeding (in clockwise fashion) sibling ports at position $j$ with bend edges $\gbe{x}{y}{i}{j}$.

\begin{figure}
  \centering
	$\vcenter{\hbox{\includegraphics[width=0.45\textwidth]{figures/node.pdf}}}$
	\caption{A $5\times3$ octilinear grid graph. Each node $\ggv{x}{y}$ has 8 ports $\gpv{x}{y}{0}, \dots, \gpv{x}{y}{7}$ which are connected to $\ggv{x}{y}$ by direct sink edges $\gse{x}{y}{0}, \dots, \gse{x}{y}{7}$. Each port is additionally connected to its $180^{\circ}$, $135^{\circ}$, $90^{\circ}$ and $45^{\circ}$ neighbor ports by bend edges $\gbe{x}{y}{i}{j}$.}
	\label{FIG:gridgraph}
\end{figure}
...
Note that $\gV = \gV^g \cup \gV^p$, $\gE = \gE^t \cup \gE^s \cup \gE^g$ and the subsets are all pairwise disjoint.
...
For example, a $45^{\circ}$ bend can be simulated by first passing $\ggv{x}{y}$ on a $180^{\circ}$ port edge, and then again on a $135^{\circ}$ port edge (Fig.~\ref{FIG:pathcircum}.3).
...
Similarly, a $90^{\circ}$ degree bend can be simulated by two cheaper $135^{\circ}$ port edges (Fig.~\ref{FIG:pathcircum}.4).

To prevent the shortcuts described above, we first introduce a constant $a \geq 0$ and offset the bend costs by $a$.
We call these updated port edge costs $\gPturnEdge{180}, \dots, \gPturnEdge{45}$ and choose $a$ in a way such that the following inequalities are fulfilled:
%
\begin{align}
...
The inequalities are fulfilled for $a = \gPturn{45} - \gPturn{135}$.

A shortest path $\gPathOcti$ on our octilinear grid graph with bend penalties will now consist of \emph{two} nodes for each original grid node: for the start and end node, the original grid node and a single port node appear in the path (Fig.~\ref{FIG:path}).
...
This shortest path $\gPathOcti = (\gv_0, \gv_1, \dots, \gv_{n'})$ thus always describes a path $\gPath = (\gv^*_0, \gv^*_2, \gv^*_4, \dots, \gv^*_{n'})$ on the corresponding original grid graph, with $|p| = n = |p'| / 2 = n' / 2$.
It has the cost
%
\begin{align}
	\gPcostOcti{\gPathOcti} = \overbrace{2 \gSinkcost}^{\makebox[0pt]{$\scriptstyle{\text{sink edges}}$}} + \underbrace{\left(n - 1\right) \cdot \gHopcost}_{\makebox[0pt]{$\scriptstyle{\text{grid hops}}$}} + \overbrace{\sum_{i=1}^{n - 1} a + c_b\left(\gv^*_{2i - 2}, \gv^*_{2i+2}\right)}^{\makebox[0pt]{$\scriptstyle{\text{offsetted bend costs}}$}}\label{EQ:cost}.
\end{align}
%
To get rid of the constant offset $a$, we set $c_{135} = 1$, $c_{90} = 1.5$ and $c_{45} = 2$, which means $a = 1$.
If we then set the cost of a single grid edge $\gHopcost = 1 = a$ and the actual grid edge costs to $\gHopcostOcti = \gHopcost - a = 0$, we can rewrite Equation~\ref{EQ:cost} as  
%
\begin{align}
	\gPcostOcti{\gPathOcti} &= 2 c_s +  \left(n - 1\right) \cdot \gHopcostOcti + \sum_{i=1}^{n - 1} \gHopcost + c_b\left(\gv^*_{2i - 2}, \gv^*_{2i+2}\right) \\
	     &= 2 c_s + \left(n - 2\right) \cdot \gHopcost + \sum_{i=1}^{n - 1} c_b \left(\gv^*_{2i - 2}, \gv^*_{2i+2}\right) \\
	     &= 2 c_s + \gPcost{\gPath} - \gHopcost = \gPcost{\gPath} + 2 \gSinkcost - 1.
\end{align}
%
As the shortest path $p'$ thus minimizes $\gPcost{p} + 2 \gSinkcost - 1$, it also minimizes $\gPcost{\gPath}$.
...
Similarly, if $\gv$ is used as a pass-through, it cannot be assigned to an input station node or be used as a pass-through by another path.
...
The only way to fulfill the constraint is thus for $\gv$ to be the source node $\gv_s$ for the input edge.
Similarly, the only way to counter an incoming edge is for $\gv$ to be the target node $\gv_t$ for the input edge.
...
The only way to fulfill the constraint is thus to set the value of $\dir{s}{e}$ to the octilinear direction.
...
\end{align}
%
If $\dir{v}{(v, u_{p})} \not< \dir{v}{(v, u_{p + 1})}$, then $\dir{v}{(v, u_{p + 1})} - \dir{v}{(v, u_{p})} \leq 0$ and the only way to fulfill Equation~\ref{EQ:dirs} is to set $\beta_p(v)$ to 1.
...
Similarly, Equations~\ref{EQ:dirneg}, \ref{EQ:bendassign} and \ref{EQ:bendsum} all add at most $8\times7$ constraints per input node, so the overall number of constraints and variables for the line bend penalty at input nodes is ${\Theta}(|V|)$.
...
ILP solution times tend to get very big for complex input graphs.
This section describes a fast method to solve the problem approximately.
...
\begin{figure}
  \centering
	...
	\vspace{-.3cm}
	\caption{Left: routing an edge on the grid graph from grid node candidates $S$ for input node $s$ to grid node candidates $T$ for input node $t$ (both within a distance $\hat{d}$ around $t$ and $s$. The grid nodes depicted as full rectangles constitute the hull of the $S$ and $T$, respectively. Right: routing an edge on the grid graph from an already settled grid node $\psi$ to a set $T$ of target node candidates.}
	\label{FIG:heur}
\end{figure}
%
...
As candidates, we use all grid nodes within a threshold distance $r$ around the original position of the input node (Fig.~\ref{FIG:heur}, left).
If both $s$ are $t$ are not settled yet, it may happen that $S$ and $T$ are not disjoint.
To prevent this, we build a local Voronoi diagram: we first define $ST = S \cup T$ and say that for each $\ggv{x}{y} \in ST$, $\ggv{x}{y}$ is moved into $S$ if it is nearer to $s$ than $t$, or into $T$ otherwise.

To prefer grid nodes as candidates that are near the original position of the input node, we offset the cost of each sink edge adjacent to the candidate grid node with the corresponding distance penalty.
...
\begin{figure}
  \centering
	...
	\caption{Left: Part of an input graph $G$. Edges $e$ and $f$ have a clockwise (green) distance of 1 at $v$, meaning that there are no edges between them in an angular, clockwise ordering. Their counterclockwise distance (blue) is 2. 
	Center: Paths $p(e)$ and $p(f)$ have already been routed. The red area is now blocked for path $p(e)$. Right: Paths $p(e)$ and $p(g)$ have already been computed, but $p(g)$ is placed in such a way that the original edge ordering cannot be respected anymore. }
	\label{FIG:paths}
\end{figure}
...
The metro maps rendered by our heuristic approach so far already look acceptable.
...
\subsection{Optimizing Distances Between Contracted Nodes}
%
\begin{figure}[b]
  \centering
	$\vcenter{\hbox{\includegraphics[width=0.479\textwidth]{figures/spring.pdf}}}$
	\vspace{-.3cm}
	\caption{Left: $k = 2$ stations are re-inserted equidistantly onto an octilinear curve with $L^{\infty}$ length $l = 2$. The hypothetical springs store a potential energy of $\frac{1}{2}c$. Right: Node $\psi(v)$ is moved to relax the spring, the potential energy is now 0.}
	\label{FIG:spring}
\end{figure}
%
As our heuristic approach contracts degree two stations in the input graph prior to finding an octilinear drawing, we have to enforce that the distance between contracted stations is at least $\hat d$ after they have been re-inserted into the final drawing. 
We reformulate hard constraint 4 as a soft constraint which is optimized during the steepest hill climbing.
We consider the octilinear drawing before the re-insertion of the contracted nodes.
For an input edge $e$ with an octilinear curve $\drawingcurve{e}$ built of $l$ grid edges and containing $k$ contracted stations, we define a spring force $F_{u, v} = c \cdot (k + 1 - l)$.
This spring is relaxed if the $L^\infty$ (grid edge) length $l$ of $e$ is such that we can insert $k$ stations equidistantly with distance 1 (Fig.~\ref{FIG:spring}).
If $F_{u, v} > 0$ (that is, if the hypothetical spring is compressed), we add the potential energy $E_{u, v} = \frac{1}{2} c \cdot (k + 1 - l)^2$ to our objective function.
Note that if we also added $E_{u, v}$ for $F_{u, v} < 0$, we would optimize for a final drawing where the length of any edge is 1.
...
It is called \emph{admissible} if it never overestimates the real cost.
...
We can then use the following heuristic:
%
\begin{align}
