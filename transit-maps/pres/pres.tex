...
\newcommand{\arrowrs}{%
\tikz [baseline=-0.5ex]{\node [draw, scale=.5, fill=blue, single arrow, minimum height=3.5ex, rotate=0] {};}}
...
\begin{frame}[t]{Goal}
	...
	$\begin{array}{l}\includegraphics[width=0.24\paperwidth]{figures/bag.pdf}\end{array}$
	\invisible<-2>{\arrowrs
	$\begin{array}{l}\includegraphics[width=0.19\paperwidth]{figures/chicago_linegraph.pdf}\end{array}$}
	\only<5->{
	\arrowrs	
	$\begin{array}{l}\includegraphics[width=0.23\paperwidth]{figures/chicago_loop.pdf}\end{array}$
	}
	...
	\begin{columns}[T]
		\begin{column}{0.3\textwidth}
			\centering \only<2->{"Bag of trips"\\\alert{(GTFS)}}
		\end{column}
		\begin{column}{0.3\textwidth}
			\centering \only<4->{Line graph}
		\end{column}
		\begin{column}{0.3\textwidth}
			\centering \only<6->{\hspace{-.6cm}Final map}
		\end{column}
	\end{columns}
...
\begin{frame}{Line graph construction}
	\begin{columns}[T]
		...
		\begin{column}{0.5\textwidth}
			\centering
			\begin{tikzpicture}
				\node[] at (0, 0) {\includegraphics[width=0.6\textwidth]{figures/chicago_linegraph.pdf}};
				\node[] at (-0.5, 0.3) {$u$};
				\node[] at (-0.5, -1.12) {$v$};
			\end{tikzpicture}
		\end{column}
	...
	Example: $\cal L = \{$\raisebox{-6pt}{\includegraphics[width=10pt]{figures/chicago_L.pdf}}$\}$, $L\left(\left(u, v\right)\right) = \{$\raisebox{-2pt}{\includegraphics[width=10pt]{figures/chicago_l.pdf}}$\}$
\end{frame}
...
\begin{frame}{Line graph construction - Input data}
	\only<1> {
		\begin{tikzpicture}[xshift=0cm, yshift=0cm, overlay]
			\node[opacity=1] at (5, -0.2) {\includegraphics[width=1.1\textwidth]{figures/freiburg_input.png}};
		\end{tikzpicture}
	}
	\only<2->{
		\begin{tikzpicture}[xshift=0cm, yshift=0cm, overlay]
			...
			\node[circle, draw=gray, line width=0.8pt,inner sep=1.1cm, path picture={\node at (path picture bounding box.center) at (0.7, 0.2) {\includegraphics[width=0.5\textwidth]{figures/freiburg_input_bbrunnen.png}};}] (a) at (8.9, -1.5) {};

			\node[circle, draw=gray, line width=0.8pt,inner sep=1.1cm, path picture={\node at (path picture bounding box.center) at (0.7, 0.2) {\includegraphics[width=0.5\textwidth]{figures/freiburg_input_runzmattenweg.png}};}] (b) at (8.9, 2.2) {};

			\node[circle, draw=gray, line width=0.8pt,inner sep=0.15cm] (enb) at (3.3, 0.05) {};
			\node[circle, draw=gray, line width=0.8pt,inner sep=0.15cm] (ena) at (5.4, -1.02) {};

			\draw[line width=0.8pt, draw=gray] (a) -- (ena);
			\draw[line width=0.8pt, draw=gray] (b) -- (enb);

			\only<3>{\node at (2, -3.8) {\textbf{$\Rightarrow$ Map construction problem}};}
		\end{tikzpicture}
	}
	\pause
	\pause
...
% --------------------------------------------------------------------------------
\begin{frame}{Line graph construction - Non-station nodes}
...
\begin{frame}{Results so far (1)}
	\only<1>{\includegraphics[width=0.82\textwidth]{figures/interim1.pdf}}%
	\pause
	\only<2>{\includegraphics[width=0.82\textwidth]{figures/interim2.pdf}}%
	\pause
	\only<3>{\includegraphics[width=0.82\textwidth]{figures/interim2b.pdf}}%
\end{frame}
...
\begin{frame}{Line-ordering optimization - Baseline ILP}
	\begin{columns}[T]
		\begin{column}{0.25\textwidth}
			...
			\only<5->{\centering\includegraphics[width=0.9\textwidth, page=2]{figures/crossing.pdf}}
		\end{column}
		\begin{column}[T]{0.75\textwidth}			
			\begin{itemize}[<+->]
				...
				\item Constraint: all $x_{elp}$ have to sum up to 1 for a single line $l$ on a single edge $e$, and for a single $p$ on a single edge
			...
			\end{itemize}
			\vspace{0.5cm}
			\only<6>{$\Rightarrow \mathcal{O}(|E|M^2)$ variables, \textcolor{red}{$\mathcal{O}(|E|M^6)$} constraints}%
			\only<-5>{\phantom {$\Rightarrow \mathcal{O}(|E|M^2)$ variables, \textcolor{red}{$\mathcal{O}(|E|M^6)$} constraints}}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}[t]{Line-ordering optimization - Improved ILP}
	...
	\end{itemize}
	\vspace{0.5cm}%
	\only<4>{$\Rightarrow \mathcal{O}(|E|M^2)$ variables, \textcolor{black!40!green}{$\mathcal{O}(|E|M^2)$} constraints}
\end{frame}
...
\begin{frame}[t]{Line-ordering optimization - Line separations}	
	\begin{columns}[T]
		\begin{column}[T]{0.45\textwidth}
			...
			\only<-4>{\phantom{1 separation}}

			\only<5->{\textcolor{red}{1 separation}}
		...
		\begin{column}[T]{0.45\textwidth}			
			...
			\only<-5>{\phantom{0 separation}}

			\only<6->{\textcolor{black!40!green}{0 separations}}
...
% --------------------------------------------------------------------------------
\begin{frame}{Line-ordering optimization - Core optimization graph}
	\centering
	...
	\begin{itemize}[<+->]
		\item Combine lines \textcolor{red}{$A,B$} that \alert{always occur together} into a single new line ($A,B \rightarrow X$)\pause
		\item Delete \textcolor{blue}{nodes with degree 2} if adjacent edges have the same lines and merge these edges\pause
		\item Remove edges \textcolor{black!30!green}{$(u, v)$} where $u$ and $v$ are \alert{termini} for all $L((u, v))$\pause\pause
		\item Cut \textcolor{blue!40!red}{edges} with $|L(e)| = 1$
	\end{itemize}
\end{frame}

% --------------------------------------------------------------------------------
\begin{frame}[t]{Rendering}
	\vspace{-0.5cm}
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\only<1->{
			\begin{figure}
				\includegraphics[width=0.6\textwidth]{figures/render_example1.pdf}
				\captionsetup{labelformat=empty}
				\caption{1. Render parallel lines}
			\end{figure}
			}
		\end{column}
		\begin{column}{0.5\textwidth}
			\only<2->{
				\begin{figure}
					\includegraphics[width=0.6\textwidth]{figures/render_example2.pdf}
					\captionsetup{labelformat=empty}
					\caption{2. Free node space}
				\end{figure}
			}
		\end{column}
	\end{columns}
	...
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\only<3->{
				\begin{figure}
					\includegraphics[width=0.6\textwidth]{figures/render_example3.pdf}
					\captionsetup{labelformat=empty}
					\caption{3. Render inner node connections}
				\end{figure}
			}
		\end{column}
		\begin{column}{0.5\textwidth}
			\only<4->{
				\begin{figure}
					\includegraphics[width=0.6\textwidth]{figures/render_example4.pdf}
					\captionsetup{labelformat=empty}
					\caption{4. Render stations}
				\end{figure}
			}
		\end{column}
	\end{columns}
...
\begin{frame}{Results so far (4)}	
	...
	\end{center}
	\only<2->{\includegraphics[trim={7cm 1.8cm 0 2.3cm},clip,width=0.45\textwidth]{figures/final.pdf}} \hspace{0.5cm} \only<3>{\includegraphics[width=0.427\textwidth]{figures/chicago_loop.pdf}}
	\pause\pause
\end{frame}
...
\begin{frame}{Evaluation - Line Ordering}	
	...
	\centering
	  \begin{tabular}{@{\extracolsep{\fill}} l r r r r r r}
	              & \multicolumn{2}{ c }{Off. map} & &  \multicolumn{2}{ c }{Our map} \\[-0.3ex]
	              \cline{2-3} \cline{5-6} \\[-1.5ex] \hline
...
\begin{frame}{Future work}	
	\begin{itemize}[<+->]
		...
		\item Faster construction times of line graph\\(current state: 1-15\,s for our test datasets)
		\item Other sources for input line graph than schedule data (e.g. OSM, \alert{work in progress})
		\item Octilinearize line graph for (non-overlay) schematic metro maps \alert{(work in progress)}
	\end{itemize}
\end{frame}
...
\begin{frame}{}	
	\begin{tikzpicture}[overlay]
	    \node at (1.5, -2.2) {\includegraphics[width=3.3\paperwidth]{figures/vvs.pdf}};
	\end{tikzpicture}

	\vspace{3.7cm}
	\hspace{5.7cm} \huge Thank you!

	\vspace{1.65cm}
	\hspace*{\fill} \footnotesize \mbox{\url{http://loom.informatik.uni-freiburg.de}} \hspace{-0.8cm}
\end{frame}
...
\begin{frame}{Line graph construction - Shared segment collapsing}
	...
	\only<1-6>{\includegraphics[width=0.4\textwidth,page=1]{figures/linegraph.pdf}}%
	\only<7>{\includegraphics[width=0.4\textwidth,page=4]{figures/linegraph.pdf}}%
	\only<8>{\includegraphics[width=0.4\textwidth,page=5]{figures/linegraph.pdf}}%
	\only<9>{\includegraphics[width=0.4\textwidth,page=2]{figures/linegraph.pdf}}%
...
% --------------------------------------------------------------------------------
\begin{frame}{Baseline ILP - Details}
	Each line must only be assigned \alert{one} position:
	\begin{equation*}
		\forall l \in L(e): \sum_{p=1}^{|L(e)|} x_{elp}=1.  \label{EQ:p_constr1_bl}
	\end{equation*}

	Each position must only be assigned once:
	\begin{equation*}
		\forall p \in  \left\{1,...,\left|L\left(e\right)\right|\right\} : \sum_{l \in L(e)} x_{elp} = 1. \label{EQ:p_constr2_bl}
	\end{equation*}
...
% --------------------------------------------------------------------------------
\begin{frame}{Stuttgart map - annotated}	
	\begin{tikzpicture}[overlay]
	    \node at (5, 0) {\includegraphics[width=0.88\paperwidth]{figures/vvs_counted.pdf}};
	\end{tikzpicture}
\end{frame}
