...
\begin{figure}
  \centering
	$\vcenter{\hbox{\includestandalone[width=0.49\textwidth]{render_examples/vvs_transit_graph}}}$
    $\vcenter{\hbox{\includegraphics[width=0.49\textwidth]{render_examples/vvs_cropped.pdf}}}$
	\caption{Left: \todo{UGLY} transit graph $T$ of the light rail network in the center of Stuttgart. Right: transit map rendered from $T$}
...
In the classic MLCM problem, each node $e=(u, v)$ is assigned two orderings $e_{u}$ and $e_{v}$ for $L(e)$, one at each node. Line crossings occur somewhere on $e$ if $e_{u} != e_{v}$ and crossings inside of nodes are prohibited. We propose an alternative model where only a single line ordering is imposed on $L(e)$, meaning the line ordering remains the same throughout $e$. This effectively disallows line crossings on edges. Instead, we explicitely allow them in nodes and favor crossings in topological nodes (Section~\ref{SEC:crossing_placement}). We call this the MLTCM problem, the additional T stands for topological. In our opinion, this model not only greatly simplifies the rendering process (Section~\ref{SEC:rendering}), but also more closely resembles the way transit maps are drawn manually. In real-world maps, crossings usually occur at positions where the network topology changes. Additionally, crossings inside of stations are commonly found in transit maps.
...
% \textbf{Example:}
% \begin{center}
% $x_{eA1} + x_{eA2} + x_{eA3} = 1$\\
% $x_{eB1} + x_{eB2} + x_{eB3} = 1$\\
% $x_{eC1} + x_{eC2} + x_{eC3} = 1$\smallskip\\
% $x_{eA1} + x_{eB1} + x_{eC1} = 1$\\
% $x_{eA2} + x_{eB2} + x_{eC2} = 1$\\
% $x_{eA3} + x_{eB3} + x_{eC3} = 1$\smallskip\\
% $x_{eA1}, x_{eA2}, x_{eA3}, x_{eB1}, x_{eB2}, x_{eB3}, x_{eC1}, x_{eC2}, x_{eC3} \in \{0, 1\}$
% \end{center}
\subsubsection{Avoiding crossings}\label{SEC:ac}
Let $A, B$ be two lines belonging to an edge $e$ over vertices $v,w$ and both extend over $w$. We distinguish two cases: Either there is an adjacent edge $e'$ with $A,B \in L(e')$ as well or $A$ and $B$ continue in different edges.
\begin{figure}
  \centering
	$\vcenter{\hbox{\includegraphics[width=0.66\textwidth]{crossing.pdf}}}$
	\caption{Line crossings of $\{A, B\}$ in the same edges (left) and into different edges (right) }
	\label{FIG:crossings}
\end{figure}
In the first case (left), $A, B$ induce a crossing if the position of $A$ is smaller than the position of $B$ in $L(e)$ but vice versa in $L(e')$. We introduce the decision variable $x_{ee'AB} \in \{0,1\}$, which should be $1$ in case a crossing is induced and $0$ otherwise. To enforce this, we create one constraint per possible crossing as follows:
\begin{align*}
	x_{eA1} + x_{eB2} + x_{e'A2} + x_{e'B1} - x_{ee'AB} &\leq 3 \\
	x_{eA1} + x_{eB3} + x_{e'A2} + x_{e'B1} - x_{ee'AB} &\leq 3 \\
	x_{eA2} + x_{eB3} + x_{e'A2} + x_{e'B1} - x_{ee'AB} &\leq 3 \\
	x_{eA2} + x_{eB1} + x_{e'A1} + x_{e'B2} - x_{ee'AB} &\leq 3 \\
	x_{eA3} + x_{eB2} + x_{e'A1} + x_{e'B2} - x_{ee'AB} &\leq 3 \\
	x_{eA3} + x_{eB2} + x_{e'A1} + x_{e'B2} - x_{ee'AB} &\leq 3.
\end{align*}
In case one of the crossings occurs, the first four variables of the corresponding constraint all have to be 1. Hence their sum equals 4 and the only way to fulfill the $\leq 3$ constraint is to set $x_{ee'AB}$ to $1$.
The objective function then minimizes the sum over all such variables $x_{ee'AB}$.
...
So far, we only optimized for the number of line crossings. However, consider the example given in Figure~\ref{FIG:linesplitting}. The number of line crossings (1) is indeed minimized in the left example. However, the intuitive information that A and B continue together after x is lost. In the right example, this information is preserved, but the number of line crossings is now 2.
...
\begin{minipage}{.48\textwidth}
  \centering
	\includegraphics[width=\textwidth]{render_examples/splitting_example_nonopt.pdf}

	\includegraphics[width=\textwidth]{render_examples/splitting_example.pdf}
...
A related problem can be seen in Figure~\ref{FIG:linesplitting2}. Both solutions have the same number of line crossings (2), but the right one clearly looks better, because the crossing is done in one pass. In both cases, we could adress the problem by punishing the separation of lines. For two adjacent edges $e$ and $e'$ and a line pair $(A, B$) that continues from $e$ to $e'$, if $A$ and $B$ were placed next to each other in $e$ (were partners in $e$) but not anymore in $e'$, we want to add some penalty to the objective function. For this, we introduce a variable $x_{eA\|B} \in \{0, 1\}$. Let $p_{eA}$ be the position of $A$ in $e$ and $p_{eB}$ the position of $B$ in $e$. We want $e_{sA\|B}$ to be $1$ if $\left|p_{eA} - p_{eB}\right| = 0$ (if they occur next to each other) and $1$ otherwise. To get the desired assignments, we add the following constraints per line pair in $e$:
\begin{align}
